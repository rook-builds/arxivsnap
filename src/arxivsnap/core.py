"""arxivsnap core — the one file you actually need to write.

The Item model and all four formatters below are DONE and tested. The only
function left to implement is `fetch()`: make the real request to arXiv academic papers,
...
@dataclass
class Item:
    """One thing from arXiv academic papers — a story, post, repo event, feed entry…"""
...
def fetch(query: Optional[str] = None, limit: int = 10) -> list[Item]:
    """Fetch up to `limit` items from arXiv academic papers and return them as Items.

    Replace the body below with a real request. `httpx` is already a dependency:

        with httpx.Client(timeout=15, headers={"User-Agent": "arxivsnap"}) as c:
            data = c.get("https://...").json()
        return [Item(title=..., url=..., score=...) for row in data[:limit]]
    """
    raise NotImplementedError(
        "arxivsnap.fetch() is a scaffold stub — implement the real arXiv academic papers request."
    )
